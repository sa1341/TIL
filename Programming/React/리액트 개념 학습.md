...
아래 예제는 특정 값이 true일 경우에만 보여지고, false일 때 태그를 안 보이게 하고 싶으면 아래처럼 작성해도 됩니다.
...
![Virtual DOM 동작 원리](../../Attached%20file/react_virtual_dom.svg)

가상 DOM은 리액트의 장점으로, 실제 모델이 바뀔 때마다 브라우저에서 DOM 트리를 생성하고 렌더링하는 과정이 반복되면 브라우저가 내부적으로 많은 연산을 하게 됩니다. 가상 DOM은 리얼 DOM에 바뀐 모델 부분을 곧바로 다시 연산하는 게 아니라, 가상으로 DOM을 그려서 리액트가 리얼 DOM과 바뀐 부분을 비교(diff)하여 바뀐 부분만 브라우저가 연산하도록 합니다.
