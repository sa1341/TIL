...
- caller/callee: 개발자가 필요한 로직을 생성하고 정의된 IDL(Interface Definition Language)로 작성하여 stub를 호출합니다.
...
IDL(Interface Definition Language)로 `protocol buffers(protobuf)`를 사용합니다. IDL만 정의하면 높은 성능을 보장하는 서비스와 메시지에 대한 소스코드가 각 언어에 맞게 자동 생성됩니다.

따라서 개발자들은 클라이언트·서버 간의 사용 언어에 구애받지 않고 생성된 코드를 사용하기만 하면 되며, 정해진 규약을 공통으로 사용하기 때문에 의사소통 비용이 감소합니다.
...
protobuf 및 HTTP/2에 대한 아주 약간의 러닝 커브가 존재합니다. 또한 일반 REST API와 다르게 메시지가 바이너리로 전달되어 개발자들이 테스트하기가 쉽지 않습니다. 이를 위한 테스트 도구로는 CLI인 **grpcurl**, GUI인 **Postman**(현재 gRPC 지원), **Kreya**, **Insomnia** 등이 있습니다.

> 예전에 많이 쓰이던 **BloomRPC**는 2022년에 아카이브(개발 중단)되었으므로, 지금은 위의 대체 도구를 사용하는 것이 좋습니다.
...
> 참고: 위 예시의 `required`는 **proto2 문법**입니다. 현재 주로 쓰는 **proto3에서는 `required`가 제거**되었고, 모든 필드가 기본적으로 optional처럼 동작합니다.

이렇게 작성된 proto 파일을 protoc 컴파일러로 컴파일하면 각 언어로 소스코드가 생성됩니다. 아래와 같은 예시로 protocol buffer가 선언되었고, 이는 통신하는 시점에 아래 이미지와 같이 인코딩되어 송수신됩니다.
