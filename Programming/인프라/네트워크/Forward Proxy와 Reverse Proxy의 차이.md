...
먼저 둘의 차이점을 알기 전에 Proxy에 대해서 간단히 정의하자면, 프록시 서버는 대행자로서 클라이언트가 자신을 통해서 다른 네트워크 서비스에 간접적으로 접속할 수 있게 해주는 컴퓨터 시스템이나 응용 프로그램을 의미합니다.

## Forward Proxy란?

클라이언트(사용자)가 인터넷에 직접 접근하는 게 아니라 Forward Proxy 서버가 요청을 받고 인터넷에 연결하여 결과를 클라이언트에게 전달해 줍니다.
...
내부 서버(WAS)에 직접 접근할 수 있게 되면 DB에도 접근이 가능해지기 때문에, 중간에 Reverse Proxy를 두고 클라이언트와 내부 서버 사이의 통신을 담당하게 합니다.

또한 내부 서버에 대한 설정으로 로드밸런싱(Load Balancing)이나 서버 확장 등에 유리합니다.
