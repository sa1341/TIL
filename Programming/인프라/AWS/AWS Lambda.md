...
AWS Lambda의 주요 장점은 **서버리스(Serverless)**라는 점입니다. 서버리스는 서버를 사용자가 관리할 필요가 없다는 의미이지, 실제로 서버가 없다는 뜻은 아닙니다. 서버 인프라 운영은 AWS 등의 클라우드 회사가 담당합니다.
...
- AWS Lambda 함수는 실행당 최대 15분 동안 하도록 구성
- Lambda는 AWS에서 서버 운영에 필요한 모든 인프라를 관리
- AWS Lambda 함수는 실행당 최대 15분 동안 하도록 구성(1초에서 15분 사이의 값으로 제한 시간을 설정)
- EC2는 Auto Scaling 기능을 사용해 서버를 확장하지만 Lambda는 사용량이 늘어나면 자동으로 용량이 확장되므로 용량 계획이 필요 없고 확장성이 뛰어납니다.
