...
k8s에서 제공하는 Service 오브젝트 타입은 총 3개입니다.
...
Pod 자체가 고유한 IP를 가지고 있지만, 다른 노드로 배포되거나 특정 사유로 인해서 휘발성으로 사라질 수 있기 때문에 변하지 않는 고유의 IP를 가진 ClusterIP 타입의 서비스를 이용해서 Pod끼리 서로 통신을 합니다.
