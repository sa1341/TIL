...
2.빌드 후 조치 탭 클릭 후 아래와 같이 Jenkins에서 build 후 생성된 jar 파일명과 실행 명령어를 작성합니다. 배포할 인스턴스가 여러 개이면 동일하게 적어줍니다.
...
Jenkins에서 Github에 있는 소스 코드를 빌드 하기 위해 빌드 환경에 아래 이미지처럼 빌드 명령어를 작성했습니다. 
...
빌드 툴을 어떤걸 쓰냐에 따라서 아래 명령어로 작성해주면 됩니다. 저는 maven으로 빌드 툴을 사용했습니다.
...
저 같은 경우에는 nginx의 reverse proxy를 이용해 로드 밸런싱을 인스턴스 3개로 설정했기 때문에 무중단 배포를 위해 30초간의 sleep 명령어를 주어 순차적으로 배포되도록 스크립트를 작성했습니다.
