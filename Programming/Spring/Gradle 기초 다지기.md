...
회사에서는 Gradle 대신 다른 빌드 툴인 Maven으로 프로젝트에서 사용하는 외부 라이브러리들을 땡겨오고 빌드를 하고 있습니다. 개인적으로 집에서 스프링 부트를 공부하면서 다른 빌드 툴을 사용하고 싶어서 Gradle을 사용한 적이 있습니다. 물론 시중에 나온 대부분의 책이 Gradle을 사용하여 프로젝트를 진행하기도 해서 무작정 사용만 하고 빌드 스크립트에 대해서 자세히 알아보지는 않아서 오늘은 Gradle에 대해서 최소한 기본적인 것을 배우고 싶어서 정리를 해보았습니다.
...
Gradle이 사용하는 폴더입니다. 작업(task)으로 생성된 파일이 저장됩니다. 이 내용을 편집하는 일은 거의 없습니다.
...
가장 중요한 것은 src 디렉토리입니다. 이 안에 개발하는 프로그램에서 사용하는 파일이 모두 저장됩니다. 다음으로 중요한 것은 `build.gradle` 파일입니다. 이것은 빌드 파일로서 빌드 처리의 내용을 작성하는 파일입니다. 이 파일은 Groovy 언어로 작성되어 있습니다.
...
jar 테스크는 그 이름대로 프로그램을 jar 파일에 모아서 저장합니다. 이는 프로젝트에 생성되는 build 디렉토리 하위에 libs 디렉토리에 저장됩니다.
...
처음에 `apply plugi:`라는 것은 Gradle 플러그인을 사용하기 위한 것입니다.
...
이것은 Apache Maven 중앙 저장소를 이용하기 위한 것입니다. Gradle은 중앙 저장소를 그대로 사용할 수 있습니다.
...
이렇게 하면 지정된 라이브러리를 클래스 경로에 추가할 수 있습니다. 컴파일 실행시까지 의존하는 라이브러리 지정에 사용합니다. 
...
>> quiet 모드로 테스크를 수행하면 많은 부분이 사라집니다. -q 옵션을 지정하고 아래와 같이 실행합니다.
...
테스크 {}에는 doFirst, doLast라는 것이 준비됩니다. 이것은 일종의 클로저입니다. 이들은 각각 다음과 같은 기능을 합니다.
 ...
 doFirst와 doLast는 각각 그 액션의 최초, 최후에 실행됩니다. 즉, `테스크의 기본적인 처리`등이 있을 때는 그전에 실행하는 것과 후에 실행하는 것을 이렇게 준비합니다.
...
```
위의 테스크를 실행하면 doFirst의 처리내용이 먼저 실행이 되고, doLast의 println 함수가 호출이 됩니다.
...
하지만 이렇게는 작동하지 않습니다. `tasks`에 있는 작업 객체 안의 메소드를 호출하여 수행해야 합니다.

모든 작업은 tasks라는 객체에 정리되어 있습니다. 예를 들어 a, b라는 테스크가 있다면 tasks.a와 tasks.b로 지정할 수 있습니다. 이 테스크 객체 안에 있는 `execute`라는 메서드를 호출하여 테스크를 수행할 수 있습니다.
...
Gradle은 표준으로 다양한 테스크가 포함되어 있는데, 이것들은 `DefaultTask`라는 클래스를 상속한 클래스로 준비되어 있습니다. 이 DefaultTask 상속 클래스는 자신의 테스크를 만들어 커스터마이징 할 수 있습니다.
...
이것은 main 안에 java 폴더를 프로젝트 폴더 외부에 복사합니다. `gradle copyJava`라고 실행해 봅니다. java 폴더가 프로젝트 폴더와 같은 위치에 `java_backup`이라는 이름으로 복사됩니다.
...
아래와 같이 jvmToolchain을 25로 지정하면 실제 JDK 25 JVM 위에서 코틀린 컴파일러를 실행한다는 의미이고 JDK 25 최신 JIT/GC/클래스 로딩 개선을 그대로 활용이 가능합니다.

하지만 jvmTarget을 17로 지정하면 바이트 코드 버전은 Java 17 클래스 파일의 버전으로 생성되는 것을 의미합니다. "17"이면 클래스 파일 major version은 61.0이 생성됩니다. 
