...
동기와 비동기의 개념에 대해서 추상적으로만 알고 있었을 뿐, 정말 누구한테 자신 있게 설명할 정도는 아니어서 최근에 괜찮은 글들을 보고 정리해봤습니다.
...
이것을 함수 호출 관점에서 동기와 비동기의 차이라고 정의할 수 있고, 대부분은 위의 동기 방식 프로그래밍에 익숙할 것이라고 생각합니다.
...
- 동기 Blocking: Application은 Kernel이 I/O 작업을 완료할 때까지 기다립니다. 그 후 결과를 직접 이용해서 이후 본인의 일을 수행합니다.
...
- 비동기 Blocking: 아직까진 I/O에서는 사례가 없지만, 멀티플렉싱 I/O를 비동기 Non-blocking이라고 하더라도 실제로 그 내부 동작 원리를 들여다보면 동기식 Non-blocking 방식입니다.